...
{
    long long sum = 0;
    int score, curNum, turn, num = 0, surplus = 0;
    unordered_set<int> winningSet;
    unordered_map<int, int> cardsInstance;
    ...
    if (inputFile.is_open())
    {
        ...
        {
            num++;
            surplus += cardsInstance[num];
            cardsInstance[num] = (surplus + 1);
            sum += cardsInstance[num];
            ...
            surplus += cardsInstance[num];
            cardsInstance[num + score + 1] -= cardsInstance[num];
        }
        inputFile.close();
        cout << sum;
    }
